...
// Cloudflare Pages Function — envío de formulario de contacto vía Mailgun
...
export async function onRequestPost(context) {
  try {
    ...
    const domain = data.domain || data.inquiry_type || "-";
    const subject = `Contacto: ${data.name} · ${domain !== "-" ? domain : data.company || "sin dominio"}`;

    const emailHTML = `
      <h2 style="font-family:sans-serif;color:#1F1B17;">Nuevo contacto desde sygnode.cl</h2>
      <table style="font-family:sans-serif;font-size:15px;border-collapse:collapse;width:100%;">
        ...
        <tr><td style="padding:8px 0;color:#6E665B;">Dominio</td><td style="padding:8px 0;">${domain}</td></tr>
      </table>
      <hr style="border:none;border-top:1px solid #E5DCC9;margin:20px 0;">
      <p style="font-family:sans-serif;font-size:15px;color:#6E665B;white-space:pre-wrap;">${data.message || "(sin mensaje)"}</p>
      <hr style="border:none;border-top:1px solid #E5DCC9;margin:20px 0;">
